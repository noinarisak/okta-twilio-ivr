...
from ivr_phone_tree_python.util.okta import (
    get_user,
    get_user_by_phone,
    get_user_factorid_by_factor_type,
    get_mfa_state_token,
    send_mfa_challenge,
    sms_mfa_verify,
)
...
def welcome():
    ...
    _message += 'Your information is {caller_name} {caller_phone_number} {caller_city} {caller_state} {caller_country}.'.format(
...
def _send_okta_push(response):
    _message = "We have sent a Okta Verify Push"
    response.say(message=_message, voice="alice", language="en-GB")
...
def _redirect_account_menu(response):
    ...
    _menu_message += "Press 1 to check balance. "
...
def _send_account_balance(response):
    _message = "Your current balance is postive 1 million dollars."
    _message += "Good bye!"
    response.say(message=_message, voice="alice", language="en-GB")
