...
def verify_okta_push():
    ...
    if _success:
        ...
        response.say(message="Sorry, there was no response from {customer_name} Verify Push. Please call again!".format(customer_name=customer_name), voice="alice", language="en-GB")
        response.hangup()
...
def _authentication(response):
    ...
    if session['factor_type'] == "sms":
        ...
        caller_factor_name = "{customer_name} Verify with Push".format(customer_name=customer_name)
    else:
...
def _send_okta_push(response):
    ...
    _message = "We have sent a {customer_name} Verify with Push. ".format(customer_name=customer_name)
    response.say(message=_message, voice="alice", language="en-GB")
...
def _lazy_support_agent(response):
    _message = "Thank you for calling {customer_name} Mobile support line.".format(customer_name=customer_name)
